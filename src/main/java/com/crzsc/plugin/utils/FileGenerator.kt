...
class FileGenerator(private val project: Project) {
    ...
    private fun genWithConfig(config: ModulePubSpecConfig) {
        val ignorePath = FileHelperNew.getPathIgnore(config)
        config.assetFileMap.forEach { (dartFileName, fileList) ->
            for (file in fileList) {
                val map = mutableMapOf<String, String>()
                generateFileMap(file, config, map, ignorePath)
                if (map.isEmpty()) {
                    continue
                }
                write2File(config, map, dartFileName.replaceFirstChar { if (it.isLowerCase()) it.titlecase() else it.toString() })
            }
        }
    }


    private fun write2File(config: ModulePubSpecConfig, map: MutableMap<String, String>, className: String) {
        ...
        map.toSortedMap().forEach {
            content.append("  String get ${it.key} => '${it.value}';\n")
        }
        content.append("\n}\n")
        val psiManager = PsiManager.getInstance(project)
        val psiDocumentManager = PsiDocumentManager.getInstance(project)
        FileHelperNew.genFile(config, className).let { generated ->
            psiManager.findFile(generated)?.let { dartFile ->
                psiDocumentManager.getDocument(dartFile)?.let { document ->
                    if (document.text != content.toString()) {
                        ...
                        showNotify("$className : assets generate succeed")
                    } else {
                        showNotify("$className : nothing changed")
                    }
                }
            }
        }
